...
module.exports = async (ctx, args) => {
    ...
    // args = username, subscription
    let username = args[0];
    let subscription = args[1];

    if (!username) return ctx.reply(await dataSets(process.env.TG_BOT_LANG, "username_is_null"))
    if (!subscription) return ctx.reply(await dataSets(process.env.TG_BOT_LANG, "subscription_is_null"))

    let reply = await ctx.reply('Deleting Subscription...');

    fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=delsub&user=${username}&sub=${subscription}`)
        .then(res => res.json())
        .then(json => {
            if (json.success) {
                ctx.deleteMessage(reply.message_id)
                ctx.replyWithHTML(`<b>Subscription has successfully been deleted!</b>`);
            }
            else {
                ctx.deleteMessage(reply.message_id)
                ctx.replyWithHTML(`<b>Subscription has failed to be deleted!</b>\n\n<b>Reason:</b> ${json.message}`);
            }
        })
};