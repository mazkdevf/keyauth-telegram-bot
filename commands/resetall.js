...
module.exports = async (ctx, args) => {
    ...
    let reply = await ctx.reply('Resetting all users...');
    ...
    fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=resetalluser`)
        ...
        .then(json => {
            ...
            if (json.success) {
                ctx.replyWithHTML(`<b>Users has successfully been reset!</b>`);
            }
            else {
                ctx.replyWithHTML(`<b>Users has failed to be reset!</b>\n\n<b>Reason:</b> ${json.message}`);
            }
        })
};