...
module.exports = async (ctx, args) => {
    ...
    let reply = await ctx.reply('Deleting Used Licenses...');
    ...
    fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=delused`)
        ...
        .then(json => {
            if (json.success) {
                ...
                ctx.replyWithHTML(`<b>Used Licenses have successfully been deleted!</b>`);
            }
            else {
                ctx.deleteMessage(reply.message_id)
                ctx.replyWithHTML(`<b>Used Licenses have failed to be deleted!</b>\n\n<b>Reason:</b> ${json.message}`);
            }
        })
};