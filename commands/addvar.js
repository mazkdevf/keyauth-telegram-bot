...
module.exports = async (ctx, args) => {
    ...
    if (!name) return ctx.reply(await dataSets(process.env.TG_BOT_LANG, "var_name_is_null"))
    if (!value) return ctx.reply(await dataSets(process.env.TG_BOT_LANG, "var_value_is_null"))
    if (!authed) return ctx.reply(await dataSets(process.env.TG_BOT_LANG, "var_authed_is_null"))
